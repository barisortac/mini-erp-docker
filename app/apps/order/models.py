...
class Order(CoreModel):
    ...
    total_package_count = models.FloatField(default=0,
    ...
    additional_discount = models.FloatField(default=0,
                                            verbose_name="Ek İndirim")
    ...
    def api_json(self):
        return {
            "order_date": self.order_date and datetime.strftime(self.order_date,
                                                                "%d.%m.%Y"),
            "delivery_date": self.delivery_date and datetime.strftime(
                self.delivery_date, "%d.%m.%Y"),
    ...
    def calculate_total_amount(self):
        ...
        total_amount = 0 or order_items.aggregate(Sum('total_amount'))[
            'total_amount__sum']
        total_amount_with_vat = 0 or order_items.aggregate(
            Sum('total_amount_with_vat'))['total_amount_with_vat__sum']
        total_quantity = 0 or order_items.aggregate(Sum('quantity'))[
            'quantity__sum']
        total_package_count = 0 or order_items.aggregate(Sum('package_count'))[
            'package_count__sum']
        if total_amount:
